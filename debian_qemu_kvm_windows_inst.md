...
3. CPU padrão: 
    Aqui temos um variedade de escolhas:
    **Padrão do aplicativo**: É a escolha padrão da interface gráfica (o Virt-Manager). Se você deixar assim, ele pede ao sistema para criar uma CPU virtual extremamente básica e genérica (geralmente chamada de qemu64). É como entregar para a VM um "motor 1.0 padrão". Ele não tem turbo, não tem injeção direta, mas funciona em absolutamente qualquer lugar. Se você pegar essa VM e colocar num computador de 15 anos atrás, ela vai ligar. O foco aqui é compatibilidade extrema, sacrificando muito o desempenho.  
    **Hipervisor padrão (A Escolha do Motor)**: Nesta opção, a interface gráfica lava as mãos e diz: "QEMU/KVM (o hipervisor), escolha você o que é melhor". Você terceiriza a decisão, mas na prática é o mesmo que **Padrão do aplicativo**.
    **Modelo da CPU host mais próxima**: Este é o sósia de prateleira, o hipervisor olha para o seu processador físico (ex: Ryzen 9), abre um catálogo de "processadores virtuais pré-fabricados" e escolhe o que for mais parecido. Se quando comprou sua máquina você disse "Vou rodar virtualização" e escolheu uma boa CPU e Memória para isso, então essa é a opção mais adequada a se usar.   
    **host-model**: Essa é a Réplica Fiel, o hipervisor mapeia detalhadamente o seu processador físico e constrói uma CPU virtual com as mesmas exatas características, flags e tecnologias (como virtualização aninhada e instruções de criptografia). É uma cópia feita sob medida do seu Ryzen 9. O desempenho é fantástico, muito próximo do real. Ainda existe uma fina camada de gerenciamento no meio, o que permite que a VM seja movida para outro servidor, desde que esse outro servidor seja quase idêntico ao seu. É o equilíbrio perfeito entre velocidade e flexibilidade corporativa.  
    **host-passthrough**: O Contato Direto, O hipervisor sai do caminho e expõe o chip físico diretamente para dentro da máquina virtual. Não há tradutores, não há cópias. O Windows ou Linux que está dentro da VM vai abrir o Gerenciador de Tarefas e ver escrito "AMD Ryzen 9". Todo o poder de fogo, cachê e instruções estão liberados. A desvantagem? A VM fica "casada" com o seu PC. Se tentar mover essa VM ligada para um computador com Intel, ela dá tela azul na mesma hora, pois o chão sumiu debaixo dos pés dela. Para uso doméstico e laboratórios de alta performance, é o rei indiscutível.  
    **maximum**: Esta é uma opção peculiar. Ela ativa absolutamente todas as características e instruções que o emulador (QEMU) consegue processar, somadas a tudo que o seu processador tem, incluindo funções experimentais.Não é feito para uso real no dia a dia, pois pode causar instabilidades. É usado exclusivamente por desenvolvedores e cientistas que estão criando sistemas operacionais novos e precisam testar o limite extremo da virtualização. Deveras, essa opção nem aparece em alguns casos, boa parte das distros, escondem esta opção.  
    **Qual devo usar?**: Se você não pretende usar a suas VMs num computador diferente do seu desktop a escolha ideal é a **host-passthrough**. Se existe a necessidade de alta-compatibilidade, transporte de VMs para processadores diferentes aí a opção pode ser outra.

![Habilitar edição de XML](img/debian_qemu_kvm_windows03.png)  

4. Firmware x86: **Padrão do sistema**. Se a CPU é o motor do carro, o Firmware é o sistema de ignição. É o primeiro software que roda quando você "liga" a máquina, responsável por acordar o hardware e entregar a chave na mão do Sistema Operacional. No mundo x86 (arquitetura de processadores de PC), nós temos basicamente dois tipos de ignição: o BIOS (antigo) e o UEFI (moderno). Escolher **Padrão do sistema** significa que se você criar uma VM Windows 11, ele assumirá UEFI, mas se escolher um Windows 7 ele assumirá BIOS, a escolha do sistema operacional determinará qual será o firmware.
